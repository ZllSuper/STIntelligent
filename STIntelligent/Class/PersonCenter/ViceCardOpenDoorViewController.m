//
//  ViceCardOpenDoorViewController.m
//  STIntelligent
//
...
@interface ViceCardOpenDoorViewController () <PCAuthCellDelegate,WXApiManagerDelegate,InviteOpenDoorTableViewdelegate>
...
@implementation ViceCardOpenDoorViewController
...
{
    ViceCardOpenDoorRequest *request = [[ViceCardOpenDoorRequest alloc] init];
...
- (BOOL)authCellTextFiledWillBegainEditing:(PCAuthCell *)authCell
{
    ...
    else
    {
        ...
        [self.pickerView setValueDidSelect:^(NSInteger index) {
            authCellWeak.inputTextFiled.text = [@[@"家人",@"亲戚",@"朋友",@"父母",@"兄妹",@"子女",@"爱人",@"其他"] objectAtIndex:index];
        }];
        return NO;
    }
}
...
- (ValuePickerView *)pickerView
{
    if (!_pickerView)
    {
        ...
        _pickerView.dataSource = @[@"家人",@"亲戚",@"朋友",@"父母",@"兄妹",@"子女",@"爱人",@"其他"];
        _pickerView.pickerTitle = @"被邀请人用途";
    }
    return _pickerView;
}
