...
- (instancetype)initWithFrame:(CGRect)frame canCancel:(BOOL)canCancel;
...
{
    LBProgressHUD *hud = [[self alloc] initWithView:view canCancel:YES];
    [view addSubview:hud];
    [hud show:animated];
    return hud;
}

+ (instancetype)showHUDCanCancelto:(UIView *)view animated:(BOOL)animated
{
    LBProgressHUD *hud = [[self alloc] initWithView:view canCancel:YES];
    [view addSubview:hud];
    [hud show:animated];
...
- (instancetype)initWithFrame:(CGRect)frame canCancel:(BOOL)canCancel
{
    ...
    if (self)
    {
        ...
        if (canCancel) {
            UIButton *cancelButton = [[UIButton alloc] initWithFrame:self.bounds];
...
- (instancetype)initWithView:(UIView *)view canCancel:(BOOL)canCancel
{
    return [self initWithFrame:view.bounds canCancel:canCancel];
}

- (instancetype)initWithWindow:(UIWindow *)window canCancel:(BOOL)canCancel
{
    return [self initWithView:window canCancel:canCancel];
}
...
{
    [LBProgressHUD hideAllHUDsForView:self.superview animated:YES];
}
