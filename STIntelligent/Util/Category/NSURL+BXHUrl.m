...
//        [self selector_exchangeImpWithOrginalSelectior:originalSelector AndSwizzledSelector:swizzledSelector];
        originalSelector = @selector(URLWithString:relativeToURL:);
        swizzledSelector = @selector(bxh_URLWithString:relativeToURL:);
//        [self selector_exchangeImpWithOrginalSelectior:originalSelector AndSwizzledSelector:swizzledSelector];
    });
}
...
//    NSString* encodedString = [[URLString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
    return [self bxh_URLWithString:URLString];
}

+ (NSURL *)bxh_URLWithString:(NSString *)URLString relativeToURL:(NSURL *)baseUrl
{
    if ([NSString stringIsEmpty:URLString])
    {
        URLString = @"";
    ...
    encodedString = [[URLString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
    return [self bxh_URLWithString:encodedString relativeToURL:baseUrl];
}
