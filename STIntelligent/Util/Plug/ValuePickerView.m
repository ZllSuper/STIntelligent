...
- (void)initSubViews
{
    ...
    self.bottomView.backgroundColor = [UIColor colorWithRed:0.88 green:0.88 blue:0.92 alpha:1.0];
...
//    self.statePicker.autoresizingMask = UIViewAutoresizingFlexibleHeight|UIViewAutoresizingFlexibleWidth;
    ...
    self.controllerToolBar.backgroundColor = Color_White;
    [self.bottomView addSubview:self.controllerToolBar];
    ...
    [self.finishBtn setTitleColor:Color_Main_Light forState:UIControlStateNormal];
    ...
    [self.cancelBtn setTitleColor:Color_Text_LightGray forState:UIControlStateNormal];
    ...
    //选择器
    self.statePicker.frame = CGRectMake(0, 40, SCREEN_WIDTH, self.pickerHeight - 44);
    ...
    self.bottomView.frame = CGRectMake(0, SCREEN_HEIGHT - self.pickerHeight, SCREEN_WIDTH, self.pickerHeight);
...
{
    self.stateStr = self.dataSource[row];
}
...
- (void)show
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.bottomView.top = SCREEN_HEIGHT - self.pickerHeight;
    }];
}
